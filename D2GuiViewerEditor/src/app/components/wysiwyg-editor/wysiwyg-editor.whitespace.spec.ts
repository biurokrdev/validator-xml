...
describe('WysiwygEditorComponent — skompilowany CSS zachowuje wielokrotne spacje (pre-wrap)', () => {
  ...
  it('nośnik tabulatora (min-width:2em) ma white-space: pre !important + tab-size: 2em (bug 14182165)', () => {
    const css = collectStyleText();
    expect(css).toMatch(
      /span\[style\*=['"]min-width:2em['"]\][^{]*\{[^}]*white-space:\s*pre\s*!important/
    );
    expect(css).toMatch(/span\[style\*=['"]min-width:2em['"]\][^{]*\{[^}]*tab-size:\s*2em/);
  });
});
